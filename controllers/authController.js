...
const createSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    expires: new Date(
      Date.now() + process.env.JWT_COOKIE_EXPIRES_IN * 24 * 60 * 60 * 1000 // convert to millseconds
    ),
    httpOnly: true, // cookie will not be accessed or modified by the browser
  };
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!email || !password) {
    return next(new AppError('Please provide with email and password!', 400));
  }

  // 2) Check if user and password is correct
  // '+' to select field which is set to false in Schema
...
exports.isLoggedIn = async (req, res, next) => {
  // 1) Get Token and check if its exits
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  await user.save({ validateBeforeSave: false }); // will deactivate all validators in our Schema
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  // BUG when copied id form mail not able to get response
  const hashedToken = crypto
    .createHash('sha256')
    .update(req.params.token)
    .digest('hex');

  // 1) get user based on encrypted token
